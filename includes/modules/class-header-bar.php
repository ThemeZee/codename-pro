...
class Codename_Pro_Header_Bar {
	...
	static function setup() {
		...
		add_filter( 'codename_hide_elements', array( __CLASS__, 'hide_header_date' ) );
	}
	...
	static function header_settings( $wp_customize ) {
		...
		$wp_customize->add_section( 'codename_pro_section_header', array(
			'title'    => __( 'Header Settings', 'codename-pro' ),
			'priority' => 20,
		...
		$wp_customize->add_control( new Codename_Customize_Header_Control(
			$wp_customize, 'codename_theme_options[header_date_title]', array(
				'label'    => esc_html__( 'Header Date', 'codename-pro' ),
		...
		$wp_customize->add_setting( 'codename_theme_options[header_date]', array(
			'default'           => false,
			'type'              => 'option',
		...
		$wp_customize->add_control( 'codename_theme_options[header_date]', array(
			'label'    => esc_html__( 'Display current date in header bar', 'codename-pro' ),
			'section'  => 'codename_pro_section_header',
			'settings' => 'codename_theme_options[header_date]',
			'type'     => 'checkbox',
			'priority' => 20,
		) );

		// Add Header Text setting.
		$wp_customize->add_setting( 'codename_theme_options[header_text]', array(
			'default'           => '',
			'type'              => 'option',
			'transport'         => 'postMessage',
		...
		$wp_customize->add_control( 'codename_theme_options[header_text]', array(
			'label'    => __( 'Header Text', 'codename-pro' ),
			'section'  => 'codename_pro_section_header',
			'settings' => 'codename_theme_options[header_text]',
			'type'     => 'textarea',
			'priority' => 30,
		) );

		// Add selective refresh for header text.
		$wp_customize->selective_refresh->add_partial( 'codename_theme_options[header_text]', array(
